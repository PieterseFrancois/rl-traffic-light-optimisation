...
class IntersectionAgent:
    def __init__(
        ...
        *,
        ...
        if not cfg.action_to_phase_index and not cfg.action_to_state_str:
            raise ValueError(
                "Provide action_to_phase_index or action_to_state_str in config"
            )
        ...
        self._nbr_buf = np.zeros(
            (self._K_eff, cfg.embed_dim), dtype=np.float32
        )  # <-- CHANGED
        ...
        for nid in cfg.neighbour_ids:
            self.comm.subscribe_embeddings(
                nid, self._on_neighbour_embedding, replay_last=True
            )
    ...
    def _on_neighbour_embedding(self, sender_id: str, emb: np.ndarray) -> None:
        ...
        if emb.shape != (self.cfg.embed_dim,):
            raise ValueError(
                f"Embedding from {sender_id} has shape {emb.shape}, expected ({self.cfg.embed_dim},)"
            )
        self._nbr_buf[i] = emb
    ...
    def _build_obs(self) -> Dict[str, np.ndarray]:
        ...
        self_raw = build_self_raw(
            self, detector_reader=self.detector_reader, device=dev
        )
        return {
            "self_raw": self_raw.detach().cpu().numpy().astype(np.float32),
    ...
    def step(
        self, deterministic: bool = True, info: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Any]:
        obs = self._build_obs()
        ...
        if self.reward is not None:
            ...
            if self.action.map_state is not None and 0 <= action < len(
                self.action.map_state
            ):
    ...
    def _log(
        self, *, t: float, action: int, phase: int, reward: Optional[float]
    ) -> None:
