...
def _setup_bidirectional_flow(num_vehicles=25):
    """Spawn equal vehicles from North→South and South→North."""
    traci.route.add("route_EW", ["EJ", "JW"])
    traci.route.add("route_WE", ["WJ", "JE"])

    for i in range(num_vehicles):
        traci.vehicle.add(f"vehN{i}", "route_EW", depart=i * 1.0)
        traci.vehicle.add(f"vehS{i}", "route_WE", depart=i * 1.0)


def run():
    _setup_bidirectional_flow(num_vehicles=25)
    ...
    plotting.plot_lane_metrics(
        ...
        data.wait,
        data.phases,
        "Avg Wait Time: Bidirectional",
        "Seconds",
        [0],
        [2],
    )
    plotting.plot_lane_metrics(
        data.steps, data.speed, data.phases, "Speed: Bidirectional", "m/s", [0], [2]
    )
    plotting.plot_lane_metrics(
        data.steps,
        data.queue,
