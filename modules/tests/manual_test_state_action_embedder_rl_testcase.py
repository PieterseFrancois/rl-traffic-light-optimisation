...
def count_features_per_lane(cfg: PreprocessorConfig) -> int:
    """Matches how your Preprocessor builds each lane row."""
    return (
        int(cfg.include_queue)
        + int(cfg.include_approach)
        + int(cfg.include_total_wait)
        + int(cfg.include_max_wait)
        + int(cfg.include_total_speed)
    )
...
class SingleTLSEnv(gym.Env):
    ...
    One env = one intersection. Each env step:
      ...
      - Build observation via Preprocessor (FLATTENED to [L*F])
      - Compute reward via RewardModule

    The policy contains the FeatureExtractor (with the embedder), so its weights train under PPO.
    """
    ...
    def __init__(
        ...
        *,
        ...
        # We'll use the *same* preprocessor config in probe & runtime
        self._preproc_config = PreprocessorConfig(
            include_queue=True,
        ...
        self._features_per_lane = count_features_per_lane(self._preproc_config)
        ...
        # Placeholder spaces (finalized in _probe_spaces())
        obs_shape, n_actions = self._probe_spaces()

        self.action_space = spaces.Discrete(n_actions)
        # Observations are *flat* (L*F)
    ...
    def _probe_spaces(self) -> tuple[tuple[int, ...], int]:
        ...
        try:
            ...
            st = pm.get_state_tensor(sm.read_state())  # [L, F]
            sample = st.flatten().cpu().numpy().astype(np.float32)  # [L*F]
    ...
    def _get_obs(self) -> np.ndarray:
        ...
        st = self.preproc.get_state_tensor(lane_state)  # [L, F]  # type: ignore
        x = st.flatten().cpu().numpy().astype(np.float32)  # [L*F]
        return x
    ...
    def step(self, action: int):
        ...
        if self.action_module.ready_for_decision():
            try:
                mask = self.action_module.get_action_mask()  # list[bool]
...
def run_baseline_episode(
    ...
    *,
    ...
    start_sumo(sumocfg, nogui=nogui, seed=seed)
...
def run_ppo_eval_episode(
    ...
    with csv_path.open("w", newline="") as f:
        ...
        while True:
            ...
            state = env.state_module.read_state()  # always read fresh
            q, wsum = collect_kpis(state)
...
def main():
    ap = argparse.ArgumentParser(
        description="Single-intersection PPO vs baseline comparison (with learnable embedder)"
    )
    ap.add_argument(
        "--sumocfg",
    ...
    save_path = outdir / "ppo_single_tls.zip"

    if args.tensorboard:
        print(
            f"[tensorboard] logs in: {Path(args.logdir, 'ppo_single_tls').resolve()}\n"
            f"Run: tensorboard --logdir {args.logdir} --port {args.tensorboard_port}"
        )

    # 1) Train PPO in the env
    env = SingleTLSEnv(
    ...
    # Feature extractor config: F must match the Preprocessor per-lane row width
    in_dim_per_lane = env._features_per_lane  # derived from env's PreprocessorConfig
    embed_hparams = EmbedderHyperparameters(
    ...
    fe_cfg = FeatureExtractorConfig(
        number_of_features=in_dim_per_lane,  # F
        embedder_hyperparams=embed_hparams,
    )

    ppo_kwargs = dict(
        policy_kwargs=dict(
            features_extractor_class=FeatureExtractor,
            features_extractor_kwargs=dict(config=fe_cfg),
        ),
        verbose=1,
        device="auto",
    )
    if args.tensorboard:
        ppo_kwargs["tensorboard_log"] = args.logdir
    ...
    print(f"[save] model saved to {save_path.resolve()}")
    ...
    # 3) Reload model and PPO evaluation rollout (no learning)
    print("[eval] reload PPO from zip and rollout…")
    # Reuse the same env instance for simplicity; you may also create a fresh one
    reloaded = PPO.load(str(save_path), env=env, device="auto")
    run_ppo_eval_episode(reloaded, env, seconds=args.episode_seconds, csv_path=ppo_csv)
