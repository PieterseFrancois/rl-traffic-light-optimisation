...
class ActionModule:
    ...
    def __init__(
        ...
        self.green_phase_map: dict[int, str] = {i: phase for i, phase in enumerate(green_phase_strings)}
    ...
    def set_phase_immediately(self, phase_index: int) -> None:
        ...
        if phase_index not in self.green_phase_map:
            raise ValueError(f"Phase index {phase_index} is not valid for TLS '{self.tls_id}'.")
        ...
        self.active_phase_memory = ActivePhaseMemory(phase_index=phase_index, start_time=self.traci.simulation.getTime())
    ...
    def set_phase(self, phase_index: int) -> None:
        ...
        if self.in_transition:
            raise RuntimeError(f"TLS '{self.tls_id}' is already in a transition. Cannot set a new phase.")
        ...
        if self.active_phase_memory.phase_index == phase_index:
            ...
            if self.timing_standards.max_green_s > 0.0 and elapsed_green_time >= self.timing_standards.max_green_s:
                # TODO: Max green time exceeded for the current phase - for now print a warning
                print(f"Warning: Maximum green time of {self.timing_standards.max_green_s}s exceeded for TLS '{self.tls_id}' on phase index {phase_index}. Current green time: {elapsed_green_time:.2f}s.")

            return  # No action needed, already in the desired phase
        
        # Check if minimum green time is enforced and not yet met    
        if self.timing_standards.min_green_s > 0.0 and elapsed_green_time < self.timing_standards.min_green_s:
            # Minimum green time not met, cannot switch phase
            # Debug log
            print(f"Minimum green time of {self.timing_standards.min_green_s}s not met for TLS '{self.tls_id}'. Current green time: {elapsed_green_time:.2f}s. Phase change to index {phase_index} ignored.")
            return
            
        # Check if a transition period is needed
        transition_needed: bool = (self.timing_standards.yellow_s > 0.0 or self.timing_standards.all_red_s > 0.0)
        if transition_needed:
            ...
            yellow_end_time = current_time + self.timing_standards.yellow_s # If no yellow, this is just current_time
            red_end_time = yellow_end_time + self.timing_standards.all_red_s # If no all-red, this is just yellow_end_time
    ...
    def update_transition(self) -> None:
        ...
        if current_time >= self.transition_memory.yellow_end_time and current_time < self.transition_memory.red_end_time:
            # Yellow phase ended, set to all red if all_red_s > 0
            if self.timing_standards.all_red_s > 0.0 and self.transition_memory.current_transition != 'all_red': # Only set all-red if not already in all-red
                self._set_all_red_phase()
    ...
    def _set_yellow_phase(self) -> None:
        ...
        yellow_phase = ''.join('y' if char in self._GREEN_CHARACTERS else char for char in current_state)
    ...
    def _set_all_red_phase(self) -> None:
        ...
        length_of_phase_string: int = len(self.green_phase_map[0])  # All phase strings should be the same length
    ...
    def _validate_green_phases(self, green_phase_strings: list[str]) -> None:
        ...
        if len(green_phase_strings) != len(set(green_phase_strings)):
            raise ValueError(f"Duplicate green phase strings found for TLS '{self.tls_id}'.")

        # Ensure only G, g, or r characters are used
        for phase_str in green_phase_strings:
            if any(character not in self._ALLOWED_GREEN_PHASE_CHARACTERS for character in phase_str):
                raise ValueError(f"Invalid character in green phase string '{phase_str}' for TLS '{self.tls_id}'. Allowed characters are {self._ALLOWED_GREEN_PHASE_CHARACTERS}.")
        ...
        for phase_str in green_phase_strings:
            if len(phase_str) != expected_length:
                raise ValueError(f"Green phase string '{phase_str}' length does not match number of controlled lanes ({expected_length}) for TLS '{self.tls_id}'.") 
    ...
    def _infer_green_phases_from_sumo(self) -> list[str]:
        ...
        for phase in phases:
            ...
            if all(char in self._ALLOWED_GREEN_PHASE_CHARACTERS for char in phase_str) and any(char in self._GREEN_CHARACTERS for char in phase_str):
    ...
    def _validate_timing_standards(self) -> None:
        ...
        if self.timing_standards.min_green_s > 0.0 and self.timing_standards.max_green_s > 0.0:
            if self.timing_standards.min_green_s > self.timing_standards.max_green_s:
                raise ValueError("Minimum green time cannot be greater than maximum green time.")
    ...
    def get_action_mask(self, ready_for_decision: bool | None = None) -> list[bool]:
        ...
        enforce_max_green: bool = (self.timing_standards.max_green_s > 0.0 and self.active_phase_memory is not None)
        if enforce_max_green and elapsed_green_time >= self.timing_standards.max_green_s:
